...
file_templates_dir="$HOME/.config/file-parser/file-templates/"
file_template="base.md"

CONFIG_FILE="$HOME/.config/file-parser/config.conf"
...
while [[ $# -gt 0 ]]; do
	case $1 in
		-h | --help)
			echo "Usage: file-parser <input_file> <output_directory> [-h|--help] [-ftf|--files-template-directory] [-ft|--file-template]";
			exit
			;;
